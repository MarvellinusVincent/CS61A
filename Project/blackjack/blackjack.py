...
def blackjack(strategy, announe = print):
    ...
    if hand_score(player_cards) > 21:
        print('Player goes bust with', player_cards, 'against a', up_card)
        return -1
    
    dealer_cards = [up_card, hole_card]
    dealer_turn(dealer_cards, deck)
    if hand_score(dealer_cards) > 21:
        print('Dealer busts with', dealer_cards)
        return 1
    else:
        print('Player has', hand_score(player_cards),
